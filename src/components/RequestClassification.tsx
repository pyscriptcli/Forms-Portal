...
export function RequestClassification({ value = "", onChange, hasError = false, compact = false }: RequestClassificationProps) {
  ...
  return (
    <section id="request-classification-section" className={`${compact ? "prime-classification-compact" : "relative mx-auto mb-8 w-full max-w-[850px] border bg-prime-white px-3 py-2.5"} overflow-visible ${!compact && (hasError ? "border-2 border-red-600 ring-2 ring-red-200" : "border-prime-rule")}`} aria-labelledby="request-classification-heading">
      {!compact && <div className={`absolute top-0 left-0 right-0 h-1 ${hasError ? "bg-red-600" : "bg-prime-gold"}`} />}
      <div className="flex flex-col gap-1.5">
        <div ref={containerRef} className="relative min-w-0 flex-1">
          <h2 id="request-classification-heading" className={compact ? "sr-only" : "mb-1 text-xs font-bold uppercase tracking-[0.12em] text-prime-blue"}>Nature of Business <span className="text-red-700" aria-hidden="true">*</span></h2>
          <label htmlFor="nature-of-transaction" className="sr-only">Nature of Business</label>
          <input id="nature-of-transaction" name="natureOfTransaction" type="hidden" value={value} required aria-invalid={hasError} />
          <button type="button" aria-labelledby="request-classification-heading" aria-haspopup="listbox" aria-expanded={isOpen} aria-controls="nature-of-transaction-options" onClick={() => setIsOpen((open) => !open)} className={compact ? `prime-classification-trigger ${isOpen ? "is-open" : ""} ${hasError ? "has-error" : ""}` : `prime-field flex w-full items-center justify-between gap-3 text-left py-2 ${hasError ? "border-red-600" : ""}`}>
            {compact ? <span className="prime-classification-copy"><span className="prime-classification-label">Nature of Business <span aria-hidden="true">*</span></span><span className="prime-classification-value">{(() => { const Icon = selected ? OPTION_ICONS[selected.value] || Receipt : Receipt; return <Icon className="h-4 w-4 shrink-0" strokeWidth={1.7} aria-hidden="true" />; })()}<span>{selected?.value || "Select category"}</span></span></span> : <span className="min-w-0"><span className="block truncate text-sm font-semibold">{selected?.value || "Select a category"}</span><span className="block truncate text-[11px] italic text-prime-ink/65">{selected?.description || "Choose a category"}</span></span>}
            <ChevronDown className={`h-4 w-4 shrink-0 transition-transform ${isOpen ? "rotate-180" : ""}`} />
          </button>
          {isOpen && (
            <div id="nature-of-transaction-options" role="listbox" aria-label="Nature of Business options" className={`prime-classification-menu absolute z-30 grid max-h-[min(70vh,31rem)] grid-cols-1 overflow-y-auto border border-prime-blue bg-prime-white p-1 shadow-lg ${compact ? "top-full mt-1" : "mt-1"}`}>
          ...
          {hasError && <p id="nature-of-transaction-error" role="alert" className="mt-1 text-xs font-semibold text-red-700">Nature of Business is required.</p>}
        </div>
      </div>
    </section>
  );
