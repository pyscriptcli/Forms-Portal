...
function RfpAppContent() {
  ...
  useEffect(() => {
    fetch("/api/admin/settings", { cache: "no-store" }).then((r) => r.json()).then((settings) => {
      ...
      if (Array.isArray(settings?.tlNames)) setTlNames(settings.tlNames);
    }).catch(() => undefined);
  }, []);
